...
export default function ActionsCitaCont({ item, navigation }: ActionsCitaProps) {
    const [loadingAction, setLoadingAction] = useState<boolean>(false)
    const { user } = useContext(AuthContext)

    // Estado global con zustand
    const { fetchCitasByContador, currentContFilter} = useCitasStore() 
    ...
    const handleDeleteCita = async () => {
        ...
        if (response) {
            ...
            await fetchCitasByContador(currentContFilter)
            navigation.navigation.goBack()
        }
    }
    ...
    return (!loadingAction ?
        <View style={{ width: '100%', alignItems: 'center', justifyContent: 'center', marginTop: 20, padding: 15, gap: 10 }}>
                ...
                user.name === item.contdr_profile.users.name &&
                        ...
                        onPress={() => {
                            navigation.navigation.replace('RescheduleCita', { item: item, path_ref: 'CitasPendingCont' });
                        }}>
